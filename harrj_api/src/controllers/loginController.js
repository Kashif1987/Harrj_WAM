...
async function UpdateLoginData(req, res) {
	return new Promise(async function (resolve, reject) {
		...
		obj.firebase_token=req.body.firebase_token

		let query = "UPDATE `user` SET ? WHERE (email_id=? or mobile_no=?) and status=?";
		let data = [obj, req.body.user_id,req.body.user_id, ACTIVE_STATUS];
	...
	validator(req.body, validationRule, {}, async (err, status) => {
		...
		if (!status) {
			...
			try {
				let now = moment();
				let todays_dt = now.format("YYYY-MM-DD HH:mm:ss");
				let query = "SELECT `id`,`name`,`email_id`, `mobile_no`,`role`  FROM `user` WHERE (email_id=? or mobile_no=?) and password=? and status=? and role=?";
				...
				if (result && result.length) {
					await UpdateLoginData(req, res,req.body.firebase_token);
					...
					res.send({
						...
						message_arabic:"تم تسجيل الدخول بنجاح",
						token: token,
						role: result[0].role,
						data: result
					});
				...
				if(result_acc_check && typeof(result_acc_check)!=='undefined' && result_acc_check.length > 0)
				{
					...
					res.send({
						...
						message_arabic:"رقم الهاتف المحمول / email_id أو كلمة المرور غير صالحة",
						data: []
					});
				}
				else{
					...
					res.send({
						...
						message_arabic:"الحساب غير موجود ، الرجاء التسجيل",
						data: []
					});
				}
...
exports.LiveLogin = async function (req, res, next) {
	const validationRule = {
		'user_id': 'required'
	}
	validator(req.body, validationRule, {}, async (err, status) => {
		...
		if (!status) {
			...
			try {
					let now = moment();
					let todays_dt = now.format("YYYY-MM-DD HH:mm:ss");
					let query = "SELECT * FROM `user` WHERE (email_id=? or mobile_no=?) and status=? and role=?";
					let data = [req.body.user_id,req.body.user_id,ACTIVE_STATUS,'customer'];
					...
					if (result && result.length) {
						...
						res.send({
							success: true,
							message: 'Acces granted , user exist !',
							// token: token,
							role: result[0].role,
							data: result
						});
						...
						let client_data = {};
						client_data.mobile_no = req.body.user_id;					
						client_data.role = 'customer';
						...
						try {
							...
							res.send({
								success: true,
								message: 'Guest user mobile no Added successfully..!',
								data: []
							});
							}
							else {
							res.send({
								success: false,
								message: 'Guest user mobile no Not Added ..!',
								data: []
							});
			...
			try {
				...
				if (result && result.length) {
					...
					res.send({
						...
						message_arabic:"معرف البريد الإلكتروني أو كلمة المرور غير صالحة",
						data: []
					});
	...
	try {
		...
		if (result) {
			...
			res.send({
				...
				message_arabic:"لم يتم تسجيل الخروج بنجاح",
				data: []
			});
		}
	} catch (error) {
			...
			try {
				...
				if (result && result.length) {
					...
					try {
							...
							mailTransporter.sendMail(mailDetails, function(err, data) {
								if(err) {
									...
									res.send({
										...
										message_arabic:"لم يتم إرسال البريد",
										data: []
									  });
									console.log(err);
								} else {
									...
									res.send({
										success: true,
										message: 'Mail sent ',
										message_arabic:"تم إرسال البريد",
										data: []
									  });
								}
							});
						...
						//   });
						  ...
						  res.send({
							...
							message_arabic:"المستخدم موجود & لم يتم إدراج Otp بنجاح",
							data: []
						  });
					...
					res.send({
						...
						message_arabic:"الحساب غير موجود",
						data: []
					});
				...
				res.send({
					...
					message_arabic:"حدث خطأ ما ... في البريد",
					data: error.message
				});
			}
			...
			try {
				...
				if (result && result.length) {
					...
					res.send({
						...
						message_arabic:"تم التحقق من OTP بنجاح",
						// token: token,
						// role: result[0].role,
						// data: result
					});
					...
					res.send({
						...
						message_arabic:"تفاصيل غير صحيحة",
						data: []
					});
			...
			try {
				...
				if (result) {
					...
					res.send({
						...
						message_arabic:"تم تحديث كلمة السر بنجاح",
						// token: token,
						// role: result[0].role,
						data: []
					});
					...
					res.send({
						...
						message_arabic:"لم يتم تحديث كلمة المرور بنجاح",
						data: []
					});
				}
			} catch (error) {
