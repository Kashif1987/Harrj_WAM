...
const NOTACCEPTED_STATUS = 'notaccepted';
...
module.exports = {
	...
	// 		else {
	// 			try {
	// 				let query = "SELECT a.bid_id,a.bidder_id,a.bid_amount,a.customer_id,a.bid_status,DATE_FORMAT(a.add_dt,'%d-%m-%Y %h:%i:%p') as add_dt,a.product_id,b.name as product_name,b.title,c.name as customer_name FROM `bids` as a left join products as b on a.product_id=b.product_id left join user as c on a.customer_id=c.id  WHERE a.bid_id = ? AND a.status=?";
	// 				// let query = "SELECT a.bid_id,a.customer_id,a.product_id,a.bid_status,DATE_FORMAT(a.add_dt,'%d-%m-%Y %h:%i:%p') as add_dt,b.name as product_name,b.title,c.name as customer_name FROM `bids` as a left join products as b on a.product_id=b.product_id left join users as c on a.customer_id=c.id  WHERE a.bid_id = ? AND a.status=?";
	...
	// 					// let query = "SELECT a.bid_id,a.bidder_id,a.bid_amount,a.customer_id,a.bid_status,DATE_FORMAT(a.add_dt,'%d-%m-%Y %h:%i:%p') as add_dt,a.product_id,b.name as product_name,b.title,c.name as customer_name FROM `bids` as a left join products as b on a.product_id=b.product_id left join user as c on a.customer_id=c.id  WHERE a.bid_id = ? AND a.status=?";
	// 					// let query = "SELECT a.bid_id,a.customer_id,a.product_id,a.bid_status,DATE_FORMAT(a.add_dt,'%d-%m-%Y %h:%i:%p') as add_dt,b.name as product_name,b.title,c.name as customer_name FROM `bids` as a left join products as b on a.product_id=b.product_id left join users as c on a.customer_id=c.id  WHERE a.bid_id = ? AND a.status=?";
	// 					let query1 = "UPDATE `bids` SET `status` =?,`mdf_by`=?,`mdf_dt`=? WHERE `bid_id` = ?";
	// 					let data1 = [ACCEPTED_STATUS, req.body.customer_id, todays_dt, req.body.bid_id];
	// 					let result1 = await model.QueryListData(query1, data1, res);

	// 					let query2 = "UPDATE `bids` SET `status` =?,`mdf_by`=?,`mdf_dt`=? WHERE `bid_id` != ? and product_id = ?";
	// 					let data2 = [NOTACCEPTED_STATUS, req.body.customer_id, todays_dt, req.body.bid_id, req.body.product_id];
	// 					let result2 = await model.QueryListData(query2, data2, res);
	// 						if (result1 || result2 && result.length) {
	// 							// result[0]['product_img'] = await model.GetProductFirstImg(result[0]['product_id']);
	// 							res.send({
	// 								success: true,
	// 								message: '',
