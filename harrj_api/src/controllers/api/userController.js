...
const bannerUploadLink =
  process.env.HOST + process.env.PORT + "/uploads/banner/";
  ...
  getinfo: async (req, res) => {
    ...
    validator(req.body, validationRule, {}, async (err, status) => {
      if (!status) {
        ...
        try {
          const query =
            "SELECT `id`, `name`, `email_id`,  `country_code_id`, `mobile_no`, `role` FROM `user` WHERE status=? and email_id=?";
          var data = [ACTIVE_STATUS,req.body.email_id];
          var result1 = await model.QueryListData(query, data, res);
