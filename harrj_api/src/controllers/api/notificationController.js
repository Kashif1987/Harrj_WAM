...
module.exports = {
	NotifyLiveAds: async (req, res) => {
		try {
            ...
            let query = "SELECT p.product_id,p.title,p.name,p.auction_type,totalbids.bidCount,p.category_id,p.sub_category_id,p.brand_id,b.brand_name,b.brand_name_arabic,p.model_id,m.model_name,m.model_name_arabic,p.year_id,p.starting_price,p.country_id,co.country_name,co.country_name_arabic,p.city_id,ci.city_name,ci.city_name_arabic, DATE_FORMAT(p.start_date_time,'%d-%m-%Y') as start_date_time,	TIME_FORMAT(p.start_time,'%h:%i %p') as start_time, DATE_FORMAT(p.end_date_time,'%d-%m-%Y') as end_date_time, TIME_FORMAT(p.end_time,'%h:%i %p') as end_time FROM products p LEFT JOIN country co ON co.country_id=p.country_id LEFT JOIN city ci ON ci.city_id=p.city_id LEFT JOIN brand b ON b.brand_id=p.brand_id LEFT JOIN model m ON m.model_id=p.model_id LEFT JOIN (select count(b1.bid_id) as bidCount,b1.product_id from bids b1 group by b1.product_id) as totalbids on totalbids.product_id = p.product_id WHERE (p.start_date_time <= ? and p.end_date_time >= ?)  and p.status=?";
			var today_date_time=moment.utc().format('YYYY-MM-DD HH:mm:ss');
			let data = [today_date_time,today_date_time,ACTIVE_STATUS];
			...
			// 	var nowtime= moment().utc().format("HH:mm:ss");
