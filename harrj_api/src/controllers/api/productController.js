...
module.exports = {
	List: async (req, res) => {
		...
		try {
			...
			let data = [ACTIVE_STATUS, todays_dtts];
				...
				try {
					...
					if (result1 && typeof result1 !== "undefined" && result1.affectedRows > 0) {
						...
						res.send({
							...
							message_arabic: "لم يضاف المنتج بنجاح",
							data: []
						});
				...
				try {
					...
					if (result) {
						...
						res.send({
							...
							message_arabic: "تم تحديث المنتج غير ناجح",
							data: []
						});
					}
				} catch (error) {
		...
		validator(req.body, validationRule, {}, async (err, status) => {
			...
			if (!status) {
				...
				try {
					...
					let data = [products_data, req.body.product_id, ACTIVE_STATUS];
					let result = await model.QueryPostData(query, data, res);
					if (req.body.meeting_status == 1) {
						let userresult= await model.QueryListData("select `firebase_token` from user where status=? and role= ? AND `firebase_token` is NOT NULL",[ACTIVE_STATUS,"customer"],res)
						let temp=[]
						userresult.forEach(element => {
							temp.push(element.firebase_token)
						});
						let productinfo=await model.QueryListData("select * from products where status=? and product_id=?",[ACTIVE_STATUS,req.body.product_id],res)
				...
				try {
					...
					if (result) {
						...
						return res.send({
							...
							message_arabic: "تمت أرشفة المنتج غير ناجح",
							data: []
						});
					}
				} catch (error) {
				...
				try {
					...
					if (result) {
						...
						return res.send({
							...
							message_arabic: "المنتج المحذوف غير ناجح",
							data: []
						});
					}
				} catch (error) {
					...
					if (result) {
						...
						if (result1) {
							if (flag == 1) {
								...
								msgarb = "Product img file removed successfully"
							}
							res.send({
								success: true,
								message: msg,
